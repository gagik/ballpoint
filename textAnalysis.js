...
function update() {
    ...
    const addValues = (pCount, wCount) => {
        chrome.storage.local.get("USER_DATA", (data) => {
            localUserData = data.USER_DATA;
            data = data.USER_DATA;
            let docInfo = data[docID].docInfo;
            if (docInfo.paragraphCount !== pCount) {
                docInfo.paragraphCount = pCount;
                chrome.storage.local.set({USER_DATA : data});
            }

            if (docInfo.wordCount !== wCount) {
                docInfo.wordCount = wCount;
                chrome.storage.local.set({USER_DATA: data});
            }
        });
    }
